...
async function waitForJSReady(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () => {
      ...
      const unconfigured = section.querySelector<HTMLElement>(
        "#ucapan-unconfigured",
      );
      const formCard = section.querySelector<HTMLElement>(
        ".ucapan-form-card",
      );
      // JS has finished when either element is shown/hidden
      return (
        (unconfigured && !unconfigured.hidden) ||
        (formCard && !formCard.hidden)
      );
    },
    { timeout: 5000 },
  );
...
async function isSupabaseConfigured(
  ...
  const form = await page
    .waitForSelector("#ucapan-form", { state: "visible", timeout: 2000 })
    .catch(() => null);
  return form !== null;
}
...
test.describe("Ucapan & Doa — Structure", () => {
  ...
  test("has 3 gradient layers", async ({ page }) => {
    const gradients = page.locator(
      "#ucapan-doa .ucapan-bg > div[class*='ucapan-bg-gradient']",
    );
  ...
  test("has left and right side decorations", async ({ page }) => {
    await expect(
      page.locator("#ucapan-doa .event-side-left"),
    ).toBeAttached();
    await expect(
      page.locator("#ucapan-doa .event-side-right"),
    ).toBeAttached();
  });

  test("has mountain silhouettes", async ({ page }) => {
    await expect(
      page.locator("#ucapan-doa .ucapan-mountain-back"),
    ).toBeAttached();
    await expect(
      page.locator("#ucapan-doa .ucapan-mountain-front"),
    ).toBeAttached();
  });

  test("has pine tree silhouettes on both sides", async ({ page }) => {
    await expect(
      page.locator("#ucapan-doa .ucapan-trees-left"),
    ).toBeAttached();
    await expect(
      page.locator("#ucapan-doa .ucapan-trees-right"),
    ).toBeAttached();
  });
  ...
  test("displays subtitle from config", async ({ page }) => {
    await expect(page.locator(".ucapan-subtitle")).toContainText(
      "Kirimkan ucapan",
    );
  });
  ...
  test("noscript message element exists but is hidden when JS is enabled", async ({
    page,
  }) => {
  ...
  test("load more button exists in the DOM", async ({ page }) => {
    await expect(page.locator("#ucapan-load-more")).toBeAttached();
  });

  test("load more wrapper exists in the DOM", async ({ page }) => {
    await expect(page.locator("#ucapan-load-more-wrapper")).toBeAttached();
  });
...
test.describe("Ucapan & Doa — Config-aware", () => {
  test("form and gallery visibility matches Supabase config", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector("#ucapan-doa");
  ...
  test("form fields are functional when Supabase is configured", async ({
    ...
    const configured = await isSupabaseConfigured(page);
    if (!configured) return; // nothing to test in non-configured env
  ...
  test("batch select has a disabled placeholder as first option", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector("#ucapan-doa");
  ...
  test("submit button exists with correct label and size", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector("#ucapan-doa");
  ...
  test("shows validation errors when submitting empty form", async ({
    ...
    await expect(page.locator("#ucapan-name-error")).toContainText(
      "tidak boleh kosong",
    );
    await expect(page.locator("#ucapan-batch-error")).toContainText(
      "tidak boleh kosong",
    );
    await expect(page.locator("#ucapan-message-error")).toContainText(
      "tidak boleh kosong",
    );
  });
  ...
  test("form fields have associated labels", async ({ page }) => {
    ...
    await expect(
      page.locator("label[for='ucapan-message']"),
    ).toBeAttached();
  });
  ...
  test("gallery section exists with title when configured", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector("#ucapan-doa");
  ...
  test("empty state message is shown initially when no messages", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForSelector("#ucapan-doa");
    ...
    const cards = page.locator("#ucapan-gallery .ucapan-card");
  ...
  test("form is usable on mobile viewport", async ({ page }) => {
    ...
    await page.waitForSelector("#ucapan-form", { state: "visible" });
